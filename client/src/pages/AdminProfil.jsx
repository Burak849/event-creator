...
const AdminProfil = ({ id }) => {
  ...
  useEffect(() => {
    if (location.state && location.state.selectedAdmins) {
      const selectedAdmins = location.state.selectedAdmins;
      setAdmins(selectedAdmins);
    } else {
      // Eğer seçili sürücü bilgisi yoksa, API'yi kullanarak sürücü bilgilerini getirin
      fetch('/api/admins/' + location.pathname.split('/').pop())
        .then(response => response.json())
